...
new_count = counter - del_count

# 傾斜屋根タイプのデータ数の算出
for (i in 1:counter) {
  yanetype = data[i, 14]
  vertex_data = data[i, 29]
...
for (i in 1:counter) {
  hyoukou = data[i, 4]    # 地盤高データの読み込み
  youto = data[i, 8]      # 建物用途データの読み込み
  story = data[i, 10]      # 階数データの読み込み
  basement = data[i, 12]  # 地下階数データの読み込み
  yanetype = data[i, 14]  # 屋根タイプの読み込み
  incline = data[i, 16]   # 屋根勾配データの読み込み
  hiratuma = data[i, 18]  # 平入り・妻入りデータの読み込み
  yanemuki = data[i, 20]  # 流れ方向の読み込み
  hisashi = data[i, 22]   # 軒庇の長さの読み込み
  keraba = data[i, 24]    # けらば幅の読み込み
  yaneatu = data[i, 26]   # 屋根厚さの読み込み
  zoning = data[i, 28]    # 用途地域の読み込み
  col_rgb[i] = F_zone_color (zoning, i)   # 関数によるcolor値(RGB)の読み込み
  
  # 頂点座標の配列変数の準備と読み込み
  vertex_org = data[i, 29] / 2  # 頂点数（屋根形態に応じる）
  ...
  # 2D建物のモデリング（QGISのDXF出力がSketchUpProで読み込めない）
  if(story == 0)
  {
    # ID番号の設定（iをidに変更）
    id = i
